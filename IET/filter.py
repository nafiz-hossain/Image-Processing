...
if __name__ == "__main__":
    ...
    cv2.imwrite('filtered.png', new_image)

    plt.plot([1], [duration_in_second], color = 'g', linestyle = 'dashed',
            marker = 'o',label = "Average execution time")
    
    plt.xticks(rotation = 10)
    plt.xlabel('Iterations')
    plt.ylabel('Average execution time (second)')
    plt.title('Duration graph IET', fontsize = 20)
    plt.grid()
    plt.legend()
    plt.show()
    plt.savefig('execution_time_plot_iet.png')


    plt.plot([1], [mse_value], color = 'g', linestyle = 'dashed',
            marker = 'o',label = "MSE values")
    
    plt.xticks(rotation = 10)
    plt.xlabel('Iterations')
    plt.ylabel('MSE values')
    plt.title('MSE calculation IET', fontsize = 20)
    plt.grid()
    plt.legend()
    plt.show()
    plt.savefig('mse_values_iet.png')
